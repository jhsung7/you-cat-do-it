...
import { getRelevantKnowledge, VetKnowledge, vetKnowledgeBase } from '../services/vetKnowledge';
...
function SymptomChecker({ catId, catName, onSave, onClose }: SymptomCheckerProps) {
  ...
  const storeAnomalies = useHealthStore((state) => state.anomaliesByCat[catId]);
  const anomalies = storeAnomalies || [];
  ...
  const [references, setReferences] = useState<VetKnowledge[]>([]);
  ...
  const handleAnalyze = async () => {
    ...
    const anomalyPrefix = anomalies.length
      ? `[${lang === 'ko' ? '최근 이상 징후' : 'Recent anomalies'}: ${anomalies
    ...
    try {
      ...
      const matchedReferences = getRelevantKnowledge(symptomText, lang, 2);
      if (matchedReferences.length > 0) {
        setReferences(matchedReferences);
      } else {
        const fallback = vetKnowledgeBase.find((item) => item.source?.url);
        setReferences(fallback ? [fallback] : []);
      }
    } catch (error) {
      console.error('Symptom analysis failed', error);
      alert(lang === 'ko' ? 'AI 분석에 실패했어요. 다시 시도해주세요.' : 'Unable to analyze. Please try again.');
    } finally {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          {/* AI 분석 결과 */}
          {analysis && (
            <div className={`border-2 rounded-lg p-4 space-y-3 ${urgencyConfig[analysis.urgency].color}`}>
              ...
              {analysis.recommendations.length > 0 && (
                <div>
                  <p className="text-sm font-semibold text-gray-800">
                    {lang === 'ko' ? '권장 조치' : 'Recommended actions'}
                  </p>
              ...
              {references.length > 0 && (
                <div className="rounded-lg border border-amber-300 bg-white/60 p-3 text-xs text-amber-900">
                  <p className="font-semibold mb-1">
                    📚 {lang === 'ko' ? '근거 자료' : 'Evidence sources'}
                  </p>
